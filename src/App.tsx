...
export default function App() {
  ...
  // Track which card flew to the table so we can unlock only after that animation completes.
  ...
  return (
    <LayoutGroup>
      <div style={{ padding: 32, fontFamily: 'sans-serif', display: 'flex', flexDirection: 'column', gap: 48 }}>

        <div>
          <p style={{ margin: '0 0 12px', color: '#888' }}>Table — {table.length} cards</p>
            ...
            {table.map(card => (
              <motion.div
                ...
                onLayoutAnimationComplete={
                  card.id === pendingCardId.current
                    ? () => { pendingCardId.current = null; setIsAnimating(false) }
                    : undefined
                }
